...
{ 
  ...
  float d=sin(fp.x*0.3+u_Time*4.0)+cos(fp.z*0.3+u_Time*2.0);
  p.y=p.y+d;
  p.xz=sim2d(p.xz,2.0);
...
vec3 camera(
  ...
  in vec2 fragCoord)
{
  vec2 vPos=-1.0+2.0*fragCoord.xy/u_Resolution.xy;
  vec3 vpn=normalize(vrp-prp);
  vec3 u=normalize(cross(vuv,vpn));
  vec3 v=cross(vpn,u);
  vec3 scrCoord=prp+vpn*vpd+vPos.x*u*u_Resolution.x/u_Resolution.y+vPos.y*v;
...
void main()
{
  ...
  //Camera animation
  vec3 vuv=vec3(0,1,0);
  vec3 vrp=vec3(u_Time*4.0,0.0,0.0);
  float mx=1.0;//iMouse.x/u_Resolution.x*PI*2.0;
  float my=0.5;//iMouse.y/u_Resolution.y*PI/2.01;
