...
{
	vec2 uv = fragCoord.xy / u_Resolution.xy*2.-1.;
	uv.y*=u_Resolution.y/u_Resolution.x;
	float st,d,col,totdist=st=0.;
	vec3 p;
	float ra=rand(uv.xy*u_Time)-.5;
	float ras=max(0.,sign(-.5+rand(vec2(1.3456,.3573)*floor(30.+u_Time*20.))));
	float rab=rand(vec2(1.2439,2.3453)*floor(10.+u_Time*40.))*ras;
	float rac=rand(vec2(1.1347,1.0331)*floor(40.+u_Time));
	float ral=rand(1.+floor(uv.yy*300.)*u_Time)-.5;
	for (int i=0; i<60; i++) {
		p=from+totdist*dir;
		d=de(p);
	...
	if (rac>.7) col=col*.7+(.3+ra+ral*.5)*mod(uv.y+u_Time*2.,.25);
	col = mix(col, .5+ra+ral*.5, max(0.,3.-u_Time)/3.);
...
void main()
{
	...
	col=pow(col,1.25)*clamp(60.-u_Time,0.,1.);
