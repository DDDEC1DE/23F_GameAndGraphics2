...
vec2 CityBlock(vec3 p, vec2 pint)
{
    ...
    //height += sin(u_Time + pint.x);	// animate the building heights if you're feeling silly
...
vec3 RayTrace(in vec2 fragCoord )
{
    ...
    // debugging camera
    float mx=0;//-iMouse.x/u_Resolution.x*PI*2.0;// + localTime * 0.05;
	float my=0;//iMouse.y/u_Resolution.y*3.14*0.5 + PI/2.0;// + sin(localTime * 0.3)*0.8+0.1;//*PI/2.01;
...
void BlockRender(in vec2 fragCoord)
{
    ...
    float frame = floor(u_Time * blockRate);
    vec2 blockRes = floor(u_Resolution.xy / blockSize) + vec2(1.0);
    // ugly bug with mod.
